...
var filterReport;
var filterText;
var idiom = new Object;
var myTips;
var ticksa = new Object;
...
function processScroll() {
    var i, scrollTop = $('html').scrollTop();
...
if (location.href.indexOf('/plugins') > 0) {
    var partes = location.href.split('/plugins/');
    basehref = partes[0] + "/";
}
...
jQuery(document).ready(function($) {
    ...
    $('#wavplayer').on('hide.bs.modal',function(){  wavesurfer.stop(); });

    $(".dopopover").popover();

    $("#menunav").bootstrapDropdownOnHover();
    ...
    if(window.location.pathname.split('/').reverse()[0]=='search.php') {

        var url = new URL(window.location.href);
        var uniqueid = url.searchParams.get("uniqueid");
        var callerid = url.searchParams.get("callerid");
        if(uniqueid!='') {
            $('#filterunique').val(uniqueid);
            $('#filterunique').trigger('keyup');
        }
        if(callerid!='') {
            $('#filtercallerid').val(callerid);
            $('#filtercallerid').trigger('keyup');
        }
    }
...
function initDataTable() {
    ...
    $('table.dattabserver').each(function() {
        ...
        rend   = $('#rend').text();
        if(rstart==rend) {
            titleDate = $('#rstart').text();
        } else {
            titleDate = $('#rstart').text()+" : "+$('#rend').text();
        } 
        ...
        if ($(this).find("tbody tr").length > 0) {
            ...
            dtable[tabid] = $(this).DataTable({
                ...
                searchDelay: 1000,
                bServerSide: true,
                sAjaxSource: ajaxscript,
                drawCallback: function(settings) {
                    $('.dopopover').popover();
                },
                lengthMenu: [
                    [10,25,50,100,-1], pagtext 
                ],
                mark: true,
                stateSave: false,
                buttons: [{
    ...
    $('table.dattab').each(function() {
        ...
        if ($(this).find("tbody tr").length > 0) {
            ...
            if(typeof(datbuttons)=='undefined') {
                datbuttons = [];
                datbuttons[tabid]=datatablebuttons;
            }

            if(typeof(datbuttons[tabid])=='undefined') {
                datbuttons[tabid]=datatablebuttons;
            }

            if ($(this).find("tbody tr:first td").length > 7) {
                for(a=0;a<datbuttons[tabid].length;a++) { 
                    if(datbuttons[tabid][a].extend=='pdfHtml5') { 
                        datbuttons[tabid][a]['orientation']='landscape';
                        datbuttons[tabid][a]['pagesize']='LEGAL';
                    } 
                }
            }
