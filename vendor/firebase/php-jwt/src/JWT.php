...
class JWT
{
    ...
    public static function decode($jwt, $key, array $allowed_algs = array())
    {
        ...
        if (empty($key)) {
            ...
            return self::response(-1,'Key may not be empty');
        }

        $tks = \explode('.', $jwt);
        if (\count($tks) != 3) {
            //throw new UnexpectedValueException('Wrong number of segments');
            return self::response(-1,'Wrong number of segments');
        ...
        if (empty($header->alg)) {
            return self::response(-1,'Empty algorithm');
    ...
    public static function response($result,$msg)
    {
        $responce=array();
