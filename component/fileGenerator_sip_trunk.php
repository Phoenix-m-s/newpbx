...
class sip_trunk_fileGenerator extends DataBase
{
    ...
    function setFieldsSip($array_fields, $defaultConfig)
    {
        ...
        foreach ($array_fields as $key => $fields) {
            $this->class_fields[$count]['trunk_sip_name']['key'] = '[' . $fields['sip_name'] . '-' . $fields['comp_name'] . ']';
            $this->class_fields[$count]['trunk_sip_name']['value'] = '';

            $this->class_fields[$count]['trunk_sip_context']['key'] = 'context';
            $this->class_fields[$count]['trunk_sip_context']['value'] = 'trunk-' . $fields['comp_name'];
    ...
    function getAllSipInfo($comp_id = '')
    {
        ...
        $append_sql = '';
        if ($comp_id != '') {
            $append_sql = "AND `tbl_sip`.`comp_id`=$comp_id ";
        }
        ...
        $sql = "
                    ...
                    FROM
                      `tbl_sip`
                      LEFT JOIN `tbl_company` ON `tbl_sip`.`comp_id` = `tbl_company`.`comp_id`
                      WHERE
                            `tbl_sip`.`trash`='0'
