...
class Extention_fileGenerator extends DataBase
{
    ...
    function setFieldsPlayVocieMail($array_fields, $defaultConfig, $count)
    {
        ...
        $this->class_fields[$count]['VoicemailPlay']['key'] = '[Gosub-VoicemailPlay-' . $comp_name . ']';
    ...
    function setFieldsQueue($array_fields, $defaultConfig, $count)
    {
        ...
        foreach ($array_fields as $key => $fields) {
            ...
            $this->class_fields[$count]['queue' . $temp_count]['key'] = 'exten';
            $this->class_fields[$count]['queue' . $temp_count]['operator'] = ' => ';
            $this->class_fields[$count]['queue' . $temp_count]['value'] = $fields['queue_ext_no'] . ',1,Answer()';
    ...
    function setFieldsOutbound($array_fields, $defaultConfig, $count)
    {
        ...
        foreach ($array_fields as $key => $fields) {
            ...
            for ($i = 1; $i <= count($prependList) - 2; $i++) {
                ...
                $this->class_fields[$count]['exten-external_If' . $temp_count . $i]['value'] = '_' . $prefixList[$i] . $match_patternList[$i] . $fields['caller_id_append'] . ',1,Execif($["${DB(Ext/${CALLERID(num)}-' . $fields[comp_name] . '/Rec/External)}" = "yes"]?Macro(record-' . $fields['comp_name'] . ',s,1))';
                ...
                foreach ($fields['siptrunk'] as $key => $sip_trunk) {
                    ...
                    $this->class_fields[$count][$sip_trunk['sip_id'] . $temp_count . $i]['value'] = '_' . $prefixList[$i] . $match_patternList[$i] . $fields['caller_id_append'] . ',n,Gosub(dialout-' . $sip_trunk['sip_name'] . '-' . $fields['comp_name'] . ',' . $prependList[$i] . '${EXTEN:' . $countPerfix . '})';
        ...
        foreach ($fieldsSipTrunk as $key => $fields) {
            ...
            $this->class_fields[$count]['record-macro-record-dialout-trunk' . $temp_count]['key'] = '['.$fields['sip_name'] . '-' . $fields['comp_name'] . ']';
    ...
    function setFieldsExtensionDialDestination($array_fields, $defaultConfig, $count)
    {
        ...
        $this->class_fields[$count]['feature-codes1' . $temp_count]['key'] = 'exten';
        $this->class_fields[$count]['feature-codes1' . $temp_count]['value'] = 's,n(vm),Gosub(VoicemailPlay-' . $fields['comp_name'] . ',noMSG,${CALLERID(dnid)})';
        $this->class_fields[$count]['feature-codes1' . $temp_count]['operator'] = ' => ';
        $temp_count++;
            ...
            if ($faild_DST == "1") {
                ...
                if ($fields['fdst_option_id'] == "7") {
                    ...
                    if ($fields['fDSTOption'] == "withOutMessage") {
                        ...
                        $this->class_fields[$count]['Macro' . $temp_count]['operator'] = ' => ';
                        $this->class_fields[$count]['Macro' . $temp_count]['key'] = 'exten';
                        $this->class_fields[$count]['Macro' . $temp_count]['value'] = $fields['extension_no'] . ',n,Macro(VoicemailPlay-' . $fields['comp_name'] . ',noMSG,' . $ExtResult['extension_no'] . ')';
    ...
    function setFieldsMacroRecord($array_fields, $defaultConfig, $count)
    {
        ...
        $this->class_fields[$count]['exten_set_path']['key'] = 'exten';
        $this->class_fields[$count]['exten_set_path']['value'] = 's,1,Set(path=' . RECORD_PATH . $comp_name . DS . "monitor" . DS . ')';
        $this->class_fields[$count]['exten_set_path']['operator'] = ' => ';
